...
public class AgentGraphGenerator {
	 ...
	 * @param highlightPairs A list where each inner list contains two strings (fromNode,
	 * toNode) representing an edge to highlight. Order matters for direction.
	...
	public static Optional<String> getAgentGraphDotSource(BaseAgent rootAgent, List<List<String>> highlightPairs) {
		log.debug("Building agent graph with root: {}, highlights: {}", rootAgent.name(), highlightPairs);
		try {
			MutableGraph graph = Factory.mutGraph("AgentGraph")
				.setDirected(true)
				.graphAttrs()
	...
	private static void buildGraphRecursive(MutableGraph graph, BaseAgent agent, List<List<String>> highlightPairs,
	...
	private static void drawNode(MutableGraph graph, Object toolOrAgent, List<List<String>> highlightPairs,
			Set<String> visitedNodes) {
		...
		MutableNode node = Factory.mutNode(name).add(Label.of(caption)).add(shape).add(LIGHT_GRAY.font());
		if (isHighlighted) {
		...
		log.trace("Added node: name={}, caption={}, shape={}, highlighted={}", name, caption, shape, isHighlighted);
	}

	/** Draws an edge between two nodes, applying highlighting if applicable. */
	private static void drawEdge(MutableGraph graph, String fromName, String toName,
			List<List<String>> highlightPairs) {
		if (fromName == null || fromName.isEmpty() || toName == null || toName.isEmpty()) {
			log.warn("Skipping edge draw due to null or empty name: from='{}', to='{}'", fromName, toName);
			return;
		}
		...
		if (highlightForward.isPresent()) {
			...
			if (!highlightForward.get()) { // If true, means b->a was highlighted, draw
											// reverse arrow
		...
		log.trace("Added edge: from={}, to={}, highlighted={}", fromName, toName, highlightForward.isPresent());
	}
	...
	/**
	 * Checks if an edge should be highlighted. Returns Optional<Boolean>: empty=no,
	 * true=forward, false=backward
	 */
	private static Optional<Boolean> isEdgeHighlighted(String fromName, String toName,
			List<List<String>> highlightPairs) {
