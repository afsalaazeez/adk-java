 ...
 * subdirectory as an "agent unit" containing pre-compiled classes. It does not perform
 * runtime compilation - agents must be pre-compiled. Agents are identified by a public
 * static field named {@code ROOT_AGENT} assignable to {@link BaseAgent}.
 *
 * <p>
 ...
 *
 * <p>
 * This loader is thread-safe and uses memoized suppliers to ensure agents are created
 * only once when first requested, improving performance for repeated access.
 *
 * <p>
 * Configuration:
 *
...
public class CompiledAgentLoader implements AgentLoader {
	...
	public CompiledAgentLoader(AgentLoadingProperties properties) {
		...
		logger.info("CompiledAgentLoader initialized with {} agents: {}", agentSuppliers.size(),
	...
	/**
	 * Loads path-based agents from the configured source directory. Similar to
	 * ConfigAgentLoader, scans directories for agent units. Each subdirectory is treated
	 * as an agent unit containing pre-compiled classes.
	 *
	 * <p>
	...
	private void loadPathBasedAgents(Map<String, Supplier<BaseAgent>> agents) {
		...
		if (!Files.isDirectory(sourceDir)) {
			logger.warn("Agent source directory does not exist: {}. Skipping path-based agent loading.", sourceDir);
			return;
		}

		logger.info("Scanning for agent units in: {}", sourceDir);

		// First check if this directory directly contains a Maven/Gradle project
		// structure
	...
	/**
	 * Loads an agent unit (directory containing pre-compiled agent classes). Similar to
	 * how ConfigAgentLoader processes agent directories.
	 */
	...
	private void loadAgentsFromDirectory(Path directory, Map<String, Supplier<BaseAgent>> agents) {
		try {
			...
			// If this is already a classes directory (e.g., target/classes), use it
			// directly
			// Otherwise, look for configured build output directories
			final Path classesDir = (directory.toString().endsWith("classes") || hasClassFiles(directory)) ? directory
					: findBuildOutputDir(directory);

			try (URLClassLoader classLoader = new URLClassLoader(new URL[] { classesDir.toUri().toURL() },
					CompiledAgentLoader.class.getClassLoader()); Stream<Path> classFiles = Files.walk(classesDir)) {
	...
	/**
	 * Finds the first existing build output directory from the configured list. Falls
	 * back to the directory itself if none of the build output directories exist.
	 */
	private Path findBuildOutputDir(Path directory) {
		...
		logger.debug("No configured build output directories found in {}, using directory itself", directory);
		return directory;
	}

	/** Attempts to load an agent from a specific class using reflection. */
	private void loadAgentFromClass(String className, ClassLoader classLoader, String unitName,
		...
		try {
			...
			if (Modifier.isStatic(rootAgentField.getModifiers())
				...
				Supplier<BaseAgent> agentSupplier = Suppliers.memoize(() -> {
					try {
						BaseAgent agentInstance = (BaseAgent) rootAgentField.get(null);
						if (agentInstance != null) {
							logger.info("Loaded path-based agent '{}' from class {} in unit {}", agentInstance.name(),
									className, unitName);
				...
				// We need to get the agent name, so we'll need to load it once to get the
				// name
				try {
					...
					if (tempAgent != null) {
						...
						if (agents.containsKey(agentName)) {
							logger.warn("Found duplicate agent name '{}'. Path-based agent from {} will overwrite"
									+ " existing agent.", agentName, unitName);
						}
					...
					logger.error("Cannot access ROOT_AGENT field in class {} to get agent name", className, e);
				}
				catch (Exception e) {
			...
			logger.warn("Class definition error loading {} from unit {}: {}", className, unitName, e.getMessage());
		}
		catch (LinkageError e) {
			logger.warn("Linkage error loading class {} from unit {}: {}", className, unitName, e.getMessage());
		}
		catch (SecurityException e) {
			logger.warn("Security error accessing class {} from unit {}: {}", className, unitName, e.getMessage());
		}
		catch (ClassCastException e) {
			logger.warn("Class {} from unit {} is not a BaseAgent: {}", className, unitName, e.getMessage());
		}
		catch (Exception e) {
			logger.error("Unexpected error loading agent from class {} in unit {}", className, unitName, e);
		}
	}
