...
public final class RecordingsLoader {
	 ...
	 *
	 * <p>
	 * In Python, GenerateContentConfig.system_instruction takes ContentUnion; In Java,
	...
	private static class ContentUnionDeserializer extends JsonDeserializer<Content> {
		...
		public Content deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
			...
			if (node.isTextual()) {
				...
				// We can't use treeToValue as it would cause recursion with the Builder
				// pattern
	...
	private static class UrlSafeBase64Deserializer extends JsonDeserializer<byte[]> {
		...
		public byte[] deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
			...
			try {
				...
				throw ctxt.weirdStringException(text, byte[].class, "Invalid Base64 encoding: " + e.getMessage());
			}
		}
	 ...
	 *
	 * <p>
	 * The mapper reads snake_case keys and attaches a mix-in so `ContentUnion` values can
	 * be either raw strings or regular `Content` objects when deserialized.
	 */
	private static ObjectMapper createYamlMapper() {
		// Custom annotation introspector that converts @JsonProperty annotation values
		// from camelCase
		// to snake_case for YAML deserialization
		JacksonAnnotationIntrospector snakeCaseAnnotationIntrospector = new JacksonAnnotationIntrospector() {
			@Override
		...
		ObjectMapper mapper = JsonMapper.builder(new YAMLFactory())
			...
			.addModule(new SimpleModule().addDeserializer(byte[].class, new UrlSafeBase64Deserializer()))
